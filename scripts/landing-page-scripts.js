...
function sizeCreator(theProduct) {
	...
	*/
	if(typeOfProduct.contains("upper-wear")  || typeOfProduct.contains("under-wear")) {  /*DOMTokenList.contains(token) returns true if the DOMTokenList contains the mentioned token */
...
function promoSticker(offPercentage) {
 	...
 	promoLogo.style.background = "#000";
 	promoLogo.style.color = "#fff";
 	promoLogo.style.display = "flex";
 	promoLogo.style.justifyContent = "center";
 	promoLogo.style.alignItems = "center";
 	promoLogo.appendChild(document.createTextNode("-" + offPercentage + "%"));
...
var promotionalProducts = document.querySelectorAll(".promotions .product-image");
promotionalProducts.forEach(function(prodImageDiv) {
 	...
 	if(prodImageDiv.parentNode.children[1].textContent.toLowerCase() == "gant" || prodImageDiv.parentNode.children[1].textContent.toLowerCase() == "les deux") {
		changePrice(prodImageDiv, 20);
		prodImageDiv.appendChild(promoSticker(20));
	}
	if(prodImageDiv.parentNode.children[1].textContent.toLowerCase() == "fila" || prodImageDiv.parentNode.children[1].textContent.toLowerCase() == "speechless") {
		changePrice(prodImageDiv, 30);
		prodImageDiv.appendChild(promoSticker(30));
	}
...
var allProducts = document.querySelectorAll(".product-image");
allProducts.forEach(function(productImageDiv){
	var labelText = "", labelbackground = "", labelTextColor = "";
	var brandName = productImageDiv.parentNode.children[1].textContent.toLowerCase();
	...
	productImageDiv.appendChild(promoMessage(labelText, labelTextColor, labelbackground)); // Puts promotional massage on selected articles
